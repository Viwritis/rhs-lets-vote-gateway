package pl.redheadsolutions.lg.lets.vote.gateway.web.rest;
...
import pl.redheadsolutions.lg.lets.vote.gateway.config.KafkaProperties;

class LetsVoteGatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        LetsVoteGatewayKafkaResource kafkaResource = new LetsVoteGatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/lets-vote-gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/lets-vote-gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
